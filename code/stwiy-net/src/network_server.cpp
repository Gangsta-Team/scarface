...
namespace gangsta::sf_net {
    Server::Server(int port, int maxClients) {
        ...
        server_ = enet_host_create(&address, maxClients, 1, 0, 0);
        if (server_ == nullptr) {
    ...
    void Server::Send(ENetPeer* peer, const RPCPacket& syncPacket) {
        ENetPacket* packet = enet_packet_create(&syncPacket, sizeof(RPCPacket), ENET_PACKET_FLAG_RELIABLE);
        enet_peer_send(peer, 0, packet);
    }

    void Server::Broadcast(ENetHost* host, const std::string& message) {
